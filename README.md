# React
Проект скомпилирован в папку site. Ничего устанавливать не надою Просто запускаем index.html из парки site.
В проекте использовал React, Redux, Context, API запросы к серверу. Данные из списка задач хранятся в store. Запросы фильтруются во избежание повторных обращений к серверу при перелистывании страниц, если страница была ранее открыта то она берется из store. Все остальные переменные состояния сохраняются в context.
--------------------------------------------------------------------------------
Тестовое задание.
Задачи состоят из:
- имени пользователя;
